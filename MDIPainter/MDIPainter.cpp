...
BEGIN_MESSAGE_MAP(CMDIPainterApp, CWinApp)
	...
	ON_COMMAND(ID_FORMVIEW_NEW, &CMDIPainterApp::OnFormViewNew)
END_MESSAGE_MAP()
// CMDIPainterApp 생성
...
BOOL CMDIPainterApp::InitInstance()
{
	...
	pDocTemplate = new CMultiDocTemplate(IDR_MDIPainterTYPE,
		RUNTIME_CLASS(CMDIPainterDoc),
		RUNTIME_CLASS(CChildFrame), // 사용자 지정 MDI 자식 프레임입니다.
		RUNTIME_CLASS(CMDIPainterView));
...
{
	POSITION positionP = GetFirstDocTemplatePosition();
	CDocTemplate* pTemplateT = GetNextDocTemplate(positionP);

	pTemplateT->OpenDocumentFile(NULL);
}
void CMDIPainterApp::OnFormViewNew()
{
	CMainFrame* pMain = (CMainFrame*)AfxGetMainWnd();
	POSITION positionP = GetFirstDocTemplatePosition();
	CDocTemplate* pTemplateT = GetNextDocTemplate(positionP);

	pMain->m_bFormView = pMain->m_bFormViewTitle = true;
	pTemplateT = GetNextDocTemplate(positionP);
	pTemplateT->OpenDocumentFile(NULL);
}
